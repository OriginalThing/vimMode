...
export const settings = definePluginSettings({
    vimChatScroll: {
        ...
        default: true,
        description: "Use 'j' and 'k' to scroll the chat in Normal mode",
    }
});

function VimScrollDisabledIcon() {
    return (
        <VimScrollIcon>
            <mask id="vim-scroll-msg-mask">
                <path fill="#fff" d="M0 0h34v34H0Z"></path>
                <path stroke="#000" strokeWidth="5.99068" d="M0 34 34 0"></path>
            </mask>
            <path fill="var(--status-danger)" d="m29.178 4.70703 1.414 1.414L6.12103 30.593l-1.414-1.415L29.178 4.70703Z" />
        </VimScrollIcon>
    );
}

const VimScrollIcon: IconComponent = ({ height = 20, width = 20, className, children }) => {
    return (
        <svg
            width={width}
            height={height}
...
const VimScrollToggle: ChatBarButtonFactory = ({ isMainChat }) => {
    const { vimChatScroll } = settings.use(["vimChatScroll"]);
    const toggle = () => settings.store.vimChatScroll = !settings.store.vimChatScroll;
